...
const MemoCommentThreadPreview: React.FC = () => {
  ...
  const { data: commentCreators } = useUsersByNames(visibleComments.map((comment) => comment.creator));
  ...
  return (
    <div className="w-full mb-2 rounded-b-2xl border border-t-0 border-border bg-gradient-to-b from-muted/30 to-background px-4 py-3">
        ...
        <div className="flex flex-col gap-2">
          {visibleComments.map((comment) => {
            ...
            return (
              <Link
                key={comment.name}
                className="group relative flex items-start gap-3 rounded-2xl border border-transparent bg-background/70 px-3 py-2 transition-all hover:border-border hover:bg-background"
...
export default MemoCommentThreadPreview;
